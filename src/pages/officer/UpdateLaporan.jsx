...
import {   getQCReportById, updateQCReport } from "../../api/qcReportApi";
...
const UpdateLaporan = () => {
  ...
  useEffect(() => {
    fetchGoods();
    getQCReportById(id).then((report) => {
      setFormData({
        title: report.title,
        description: report.description,
        report_notes: report.report_notes,
        report_date: dayjs(report.report_date).toISOString(),
        items: report.qcReportItems.map((item) => ({
      ...
      setFileList(report.photo_urls.map((url, index) => ({ uid: String(index), url })));
    });
  }, [id, fetchGoods]);
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      await updateQCReport(id, formData);
  ...
  return (
    <div style={{ padding: "2rem", background: "#fff", borderRadius: "8px" }}>
      <h2 style={{ textAlign: "center" }}>Update Laporan</h2>
      <Form layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item>
          <Button type="primary" htmlType="submit" block loading={loading}>Update Laporan</Button>
        </Form.Item>
      </Form>
    </div>
  );
